...
export default function TerminalPage() {
  ...
  const [output, setOutput] = useState<string[]>(['Welcome to the enhanced terminal. Type your commands below.'])
  ...
  const executeCommand = async (command: string): Promise<string> => {
    ...
    switch (command.toLowerCase().trim()) {
      case 'help':
        return 'Available commands: help, echo, date, clear, theme'
      case 'echo':
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (input.trim() && !isExecuting) {
      ...
      try {
        ...
        const updatedOutput = commandOutput ? [...newOutput, commandOutput] : newOutput
  ...
  const handleDownloadHistory = () => {
    const historyText = commandHistory.map(cmd => `${cmd.timestamp} - ${cmd.command} (${cmd.status})\n${cmd.output}`).join('\n\n')
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="md:col-span-2">
          ...
          <CardContent className="p-4">
            <ScrollArea className={cn("h-[60vh] w-full rounded-md border p-4 font-mono text-sm", THEMES[selectedTheme])} ref={outputRef}>
                    ...
                    <TableBody>
                      {commandHistory.map((cmd, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <span className={
                              cmd.status === 'success' ? 'text-green-500' :
                ...
                <ScrollArea className="h-[50vh]">
                  {commandHistory.length > 0 && (
                    <div className="space-y-4">
                      ...
                      <p><strong>Command:</strong> {commandHistory[0].command}</p>
                      <p><strong>Executed at:</strong> {commandHistory[0].timestamp}</p>
                      <p><strong>Status:</strong> <span className={
                        commandHistory[0].status === 'success' ? 'text-green-500' :
                        commandHistory[0].status === 'error' ? 'text-red-500' :
                        'text-yellow-500'
                      }>{commandHistory[0].status}</span></p>
                      <p><strong>Output:</strong></p>
                      <pre className="bg-gray-100 p-2 rounded text-sm whitespace-pre-wrap">{commandHistory[0].output}</pre>
                    </div>
                  )}
                </ScrollArea>
              </TabsContent>
