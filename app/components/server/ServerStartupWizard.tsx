  ...
  const canProceed = () => {
    switch (step) {
      case 0:
        return config !== null;
      case 1:
        return config && config.name && config.port;
      case 2:
        return Object.values(config).some(
          (value: any) => value && value.driverId
        );
      case 3:
        return Object.values(config).some(
          (value: any) => value && value.deviceId
        );
      default:
        return true;
    }
      ...
      <CardContent className="p-4 sm:p-6 md:p-8">
        ...
        <div className="mb-4 flex justify-end">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="mock-mode" className="text-sm sm:text-base">
              Mock Mode
            </Label>
          </div>
        </div>
        <CurrentStep
          ...
          onComplete={handleNext} // 添加 onComplete 属性
        />
        <div className="flex justify-between mt-4 sm:mt-6">
          {step > 0 && (
            <Button onClick={handlePrevious} className="text-sm sm:text-base">
              Previous
            </Button>
          )}
          {step < steps.length - 1 ? (
            ...
            <Button
              onClick={() => console.log("Start server")}
              className="text-sm sm:text-base"
            >
              Start Server
            </Button>
