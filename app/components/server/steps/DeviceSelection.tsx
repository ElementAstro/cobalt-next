...
export function DeviceSelection({
      ...
      try {
        const allDevices = await Promise.all(
          Object.entries(config).map(
            async ([category, value]: [string, any]) => {
              if (value?.driverId) {
                const categoryDevices = await fetchDevices(
                  value.driverId,
                  isMockMode
                );
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-4 p-4 sm:p-6 md:p-8">
          ...
          <TabsContent key={category} value={category}>
            {config[category]?.driverId ? (
              <Select
                ...
                value={config[category]?.deviceId || ""}
              >
