...
import type { CustomOptions } from '@/types/guiding/config'
import { useMeasurements } from './hooks/use-measurements'
import { useAppState } from './hooks/use-app-state'
import { useStore } from './store/useStore'

export default function StarGuiding() {
  const { isLandscape, isMobile } = useAppState()
  const {
    isRunning,
    setIsRunning,
    measurePeriodical,
  ...
  const handleCustomOptionChange = (option: keyof CustomOptions, value: number) => {
  ...
  return (
    <Card className={`w-full mx-auto ${isLandscape && isMobile ? 'h-screen overflow-auto' : 'max-w-4xl'} p-2`}>
              ...
              <div className="grid gap-3 py-3">
                <div className="grid grid-cols-4 items-center gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleCustomOptionChange('snrThreshold', Number(e.target.value))}
                    className="col-span-3 h-8 text-xs"
                  />
                </div>
                <div className="grid grid-cols-4 items-center gap-2">
                  <Label htmlFor="measurementInterval" className="text-right text-xs">
                    测量间隔 (ms)
                  </Label>
                  <Input
                    ...
                    onChange={(e) => handleCustomOptionChange('measurementInterval', Number(e.target.value))}
                    className="col-span-3 h-8 text-xs"
                  />
                </div>
                <div className="grid grid-cols-4 items-center gap-2">
                  <Label htmlFor="autoStopDuration" className="text-right text-xs">
                    自动停止时间 (ms)
                  </Label>
                  <Input
                    id="autoStopDuration"
                    ...
                    onChange={(e) => handleCustomOptionChange('autoStopDuration', Number(e.target.value))}
                    className="col-span-3 h-8 text-xs"
                  />
                </div>
              </div>
          ...
          >
            ...
            <div className={`grid ${isLandscape && isMobile ? 'grid-cols-1 gap-2' : 'grid-cols-1 md:grid-cols-2 gap-3'}`}>
              ...
              <motion.div
                ...
                className={isLandscape && isMobile ? 'col-span-1' : 'col-span-1 md:col-span-2'}
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <Table>
                    <TableBody>
                      <TableRow>
                        <TableCell className="font-medium">赤经，峰值</TableCell>
                        <TableCell>{starPosition.redPeak.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">赤经，峰峰值</TableCell>
                        <TableCell>{starPosition.bluePeak.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">赤经最大漂移率</TableCell>
                        <TableCell>{starPosition.maxDriftRate.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">赤纬漂移速率</TableCell>
                        <TableCell>{starPosition.driftSpeed.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">极轴误差</TableCell>
                        <TableCell>{starPosition.polarAxisError.toFixed(2)}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                  <Table>
                    <TableBody>
                      <TableRow>
                        <TableCell className="font-medium">赤纬，峰值</TableCell>
                        <TableCell>{starPosition.greenPeak.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">赤经漂移率</TableCell>
                        <TableCell>{starPosition.driftRate.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">无导星极限曝光时间</TableCell>
                        <TableCell>{starPosition.noStarExposureTime.toFixed(2)}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">赤纬回差</TableCell>
                        <TableCell>{starPosition.periodicalError.toFixed(2)}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                </div>
            ...
            <div className="flex items-center space-x-2 text-xs">
              <div className="flex items-center space-x-1">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setMeasurePeriodical(checked as boolean)}
                />
                <label
                  htmlFor="measure"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm">查看之前的</Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent>
